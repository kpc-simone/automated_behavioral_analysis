...
for feature_csv in feature_csvs:
    ...
    t_sh0_manual = a_df['shadowON-abs'].iloc[0]    
    fdf['shadow-cleaned'] = fdf['shadow']
    fdf.loc[:,'shadow-cleaned'].iloc[:int(t_sh0_manual*FPS)] = False
    ...
    # align shadows against manual analysis
    
    tidx = fdf.index[0]
    time = fdf['time'].iloc[0]
    shadow = fdf['shadow-cleaned'].iloc[0]
    while(shadow == False):
        t_sh0_auto = fdf['time'].iloc[tidx]
        shadow = fdf['shadow-cleaned'].iloc[tidx]
        tidx += 1  
    print(t_sh0_manual,t_sh0_auto)    
    fdf['time-aligned'] = fdf['time'] + t_sh0_manual - t_sh0_auto
    
    axes[0,0].plot(fdf['time'],fdf['shadow'],color='k')
    axes[0,0].set_ylabel('shadow')
    
    axes[1,0].plot(fdf['time-aligned'],fdf['shadow-cleaned'],color='k')
    ...
    axes[1,0].set_xlabel('Time (aligned) (s)')
    plt.show()
    
    fdf.to_csv('../fx_analyses/{}-aligned.csv'.format(feature_csv_filename[:-4]))