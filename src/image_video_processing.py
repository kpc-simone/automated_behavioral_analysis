...
from skimage.morphology import disk, dilation, closing, binary_erosion, binary_dilation
import numpy as np
import math
import cv2
...
def generateBackgroundModel(vidcap,roi,pos_0,FPS):
    ...
    while vidcap.isOpened():
        ...
        cv2.accumulateWeighted(frame,wa,alpha=0.001)
        diff = frame - wa
        ...
        if abs(diff_metric) < 5.0:
            ...
            print('built background model in {} s of video'.format(t_built))
...
def thresholdImage(method='BSAT',method_args=None):
    ...
    if method == 'BSAT':
        ...
        diff = ( ( diff - diff.min().min() ) / ( diff.max().max() - diff.min().min() ) ) * 255
        diff = inv_offset - diff
        at2 = cv2.adaptiveThreshold(np.uint8(diff),inv_offset,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY,at_blocksize,at_level)
        at = inv_offset - at2
        at = at > 0
