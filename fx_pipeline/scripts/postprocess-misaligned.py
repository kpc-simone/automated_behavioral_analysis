...
for file in files:
    ...
    df['escape-filtered'] = getEscapeBouts(df['speed-filtered'],df['hide'],df['shadow'],v_threshold1=v_threshold1,v_threshold2=v_threshold2)
    df['hide-filtered'] = getHideBouts(df['x-corr-filtered'],df['y-corr-filtered'],y_thresh = 0.130,x_thresh = 0.01)

    axes[5,0].plot(df['time'],df['freeze-filtered'],color='#92ab87FF')
    axes[5,0].set_ylabel('freeze')
    axes[6,0].plot(df['time'],df['escape-filtered'],color='#55868CFF')
    axes[6,0].set_ylabel('escape')
    axes[7,0].plot(df['time'],df['hide-filtered'],color='#675159FF')
    axes[7,0].set_ylabel('hide')

    if 'rearing' in behaviors:
        axes[-2,0].plot(df['time'],df['rear'],color='#EBBC6Fff')
        axes[-2,0].set_ylabel('rear')

    axes[-1,0].plot(df['time'],df['shadow'],color='k')
